...
upload_data <- function(input, output, session) {
  ...
  data_list <- shiny::reactive({
    ...
    hold <- selected_files() %>% dplyr::pull(datapath)
    purrr::map(hold,
               rio::import,
               setclass = "tibble") %>%
      purrr::set_names(hold)
  })
  ...
  selected_files_metadata <- shiny::reactive({
    shiny::req(length(data_list()) > 0, data_dims())
    purrr::map_dfr(names(data_list()), # %>% purrr::set_names(basename(.)),
                   fs::file_info) %>% #, .id = "index") %>%
      dplyr::mutate(path = as.character(path)) %>%
      dplyr::transmute(
        index = dplyr::row_number(), # index,
